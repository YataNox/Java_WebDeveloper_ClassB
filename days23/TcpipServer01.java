...
public class TcpipServer01 
{
	public static void main(String[] args)
	{
		...
		try 
		{
			...
			// 여기까지 완료되면 서버가 클라이언트의 연결 요청을 기다리며, 수락할 준비가 완료됩니다.
		...
		// 서버 소켓은 준비 완료 이후 클라이언트의 연결 요청이 올 때까지 실행을 잠시 멈추고 계속 기다립니다.
		
		try
		{
			System.out.println(f.format(new Date()) + " 연결 요청을 기다립니다.");
			Socket s = ss.accept();
			// 클라이언트의 연결 요청이 오면 클라이언트 소켓과 통신할 새로운 통신용
			// 소켓을 연결정보와 함께 생성하고, 서버 소켓이 연결해놓은 클라이언트 연결정보를 전달합니다.
			System.out.println(f.format(new Date()) + s.getInetAddress() + "로부터 연결 요청이 들어왔습니다.");
