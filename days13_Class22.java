...
class MemberCall
{
	...
	int iv2 = sv;
	// static int sv2 = iv; 
	// 에러 : iv가 언제 생성될지 모르므로, sv2에 값을 전달할 수 없다.
	
	static int sv2 = new MemberCall().iv;
	// 임시 방편으로 객체 생성 후 변수를 사용하는 예
	// 억지로라도 인스턴스(멤버)변수로 스태틱 변수의 값을 초기화하고자 한다면
	// 위와 같은 방법을 씁니다. 레퍼런스 변수 없는 new 인스턴스를 만들어서 멤버 변수 사용.
	...
	static void staticMethod()
	{
		...
		// 억지로라도 스태틱 메서드가 멤버 메소드를 호출하려면 객체를 만들고 사용가능
	}
