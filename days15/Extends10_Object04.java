...
class Student
{
	...
	public boolean equals(Object obj)
	{
		...
		// 부모 클래스의 레퍼런스는 자식 인스턴스를 가졌을 때, 부모가 물려준 멤버는 접근이
		// 가능하지만, 자식이 새로 만든 멤버는 접근이 불가능합니다.
		// 따라서 this와 obj가 비교가 되려면, obj를 Student 형으로 변환해야합니다.
		// Student s = (Student)obj;
		// 또한 전달된 obj가 현재로선 어떤 인스턴스를 갖고 있는지 모르기때문에,
		// instanceof를 이용하여 형변환이 가능한지 보고 변환합니다.
		// 이상한 클래스가 왔으면 그냥 바로 return(메서드 종료)
		boolean flag_name = this.name.equals(s.name);
		boolean flag_age = this.age == s.age;
		if(flag_name && flag_age)
			return true;
		else
			return false;
	}
